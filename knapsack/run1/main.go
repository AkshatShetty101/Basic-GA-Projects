...
func main() {
	...
	maxWeight := 10.0
	maxVolume := 4.0
	knapsack = create(knapsack, items, maxWeight, maxVolume)
	parentFitness := getFitness(knapsack)
	display(knapsack, parentFitness, maxWeight, maxVolume, start)
	for {
		child := mutate(knapsack, items, maxWeight, maxVolume)
		childFitness := getFitness(child)
		if compareFitness(childFitness, parentFitness) {
			knapsack = child
			parentFitness = childFitness
			display(child, childFitness, maxWeight, maxVolume, start)

		}
		if childFitness.totalValue >= optimal.totalValue {
			break
		}
	}
}
...
func display(k []itemQuantity, f fitness, maxW float64, maxV float64, start time.Time) {
	fmt.Println("-----------------------------------------------")
