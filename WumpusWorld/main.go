...
func move(player gene) gene {
	...
	if len(boardStatus[0]) > 0 {
		...
		if boardStatus[0] == "breeze" && len(boardStatus) == 1 {
			if next := forward(player); checkMoveViability(next) && (pitIssues)[next.c] == "-" {
				return next
			}
		...
		if boardStatus[0] == "stench" && len(boardStatus) == 1 {
			if next := forward(player); checkMoveViability(next) && (wumpusIssues)[next.c] == "-" {
				return next
			}
		...
		move := forward(player)
		if checkMoveViability(move) == false {
			return lOrR(possibleLeft, possibleRight, player)
		}
		return move
	}
...
func checkMoveViability(player gene) bool {
	if player.c.x > -1 && player.c.x < 4 && player.c.y > -1 && player.c.y < 4 {
		return true
	}
	return false
